...
const fallbackHadithCollection = [
    ...
    { text: "عن أبي عبد الله الصادق (عليه السلام) قال: «حَدِيثِي حَدِيثُ أَبِي، وَحَدِيثُ أَبِي حَدِيثُ جَدِّي...»", source: "الكافي الشريف - ج1 ص53" },
    { text: "قال الإمام علي بن الحسين السجاد (عليه السلام): «لَوْ يَعْلَمُ النَّاسُ مَا فِي طَلَبِ العِلْمِ لَطَلَبُوهُ وَلَوْ بِسَفْكِ المُهَجِ...»", source: "الكافي الشريف - ج1 ص35" }
];
...
function normalizeArabicText(text) {
    if (!text) return "";
    return text.replace(/[\u064B-\u065F\u0670ـ]/g, "").replace(/[أإآ]/g, "ا").replace(/ة/g, "ه").toLowerCase().trim();
}
...
function createArabicSearchRegex(rawQuery) {
    ...
    let wordPatterns = words.map(w => {
        ...
        for (let i = 0; i < w.length; i++) {
            ...
            else if (c === "ي") p += "ي" + tashkeel;
            else if (c === "ى") p += "ى" + tashkeel;
    ...
    try { return new RegExp(fullPattern, "gim"); } catch(e) { return null; }
}
...
const compoundMap = { "العاشر": 10, "التاسع": 9, "الثامن": 8, "السابع": 7, "السادس": 6, "الخامس": 5, "الرابع": 4, "الثالث": 3, "الثاني": 2, "الأول": 1, "الاول": 1 };
...
function getVolumeNumber(vol) {
    ...
    if (idMatch && !isNaN(parseInt(idMatch[1], 10))) return parseInt(idMatch[1], 10);
...
function getGroupName(book, bookId) {
    ...
    if (book.pdf_url || lowerId.includes("mkh") || normTitle.includes("مخطوط") || normTitle.includes("نسخه خطيه")) return rawTitle;
    if (lowerId.startsWith("bhr") || normTitle.includes("بحار الانوار")) return "بحار الأنوار";
    if (lowerId.startsWith("kafi") || normTitle.includes("الكافي")) return "الكافي الشريف";
...
function getBookCategory(book) {
    ...
    if (book.pdf_url || title.includes("مخطوط")) return "المخطوطات والوثائق التراثية";
    if (title.includes("تفسير") || title.includes("قرآن")) return "التفسير وعلوم القرآن";
    if (title.includes("حديث") || title.includes("الكافي") || title.includes("بحار") || title.includes("وسائل")) return "الحديث الشريف ومصادره";
    if (title.includes("دعاء") || title.includes("صحيفة") || title.includes("زيارة")) return "الأدعية والزيارات";
    if (title.includes("فقه") || title.includes("احكام")) return "الفقه والأحكام";
...
async function loadLibraryManifest() {
    ...
    try {
        ...
        if (Object.keys(allBooksManifest).length === 0) throw new Error("لم يتم العثور على أي بيانات");
        ...
        console.error(err);
        container.innerHTML = `<div style="color:#ff6b6b; grid-column: span 2; text-align: center; padding: 20px;">تعذر تحميل الفهارس.</div>`;
    }
}

// ==================== بناء واجهة الكتب مع السلايدر التلقائي ====================
let heroSliderTimer = null; // تعريف متغير السلايدر مرة واحدة فقط هنا
...
function processAndRenderBooks(data) {
    ...
    sortedGroupTitles.forEach(groupTitle => {
        ...
        let subtitle = isPdfManuscript ? `${mainBook.total_pages || 0} لوحة (مخطوط PDF)` : (isSeries ? `${booksInGroup.length} أجزاء` : `${mainBook.total_pages || 0} صفحة`);
...
function renderCatalogAccordion() {
    ...
    const sortedCategories = Object.keys(categoriesMap).sort((a, b) => a.localeCompare(b, 'ar', { numeric: true }));
    ...
    sortedCategories.forEach((catName, index) => {
        ...
        items.forEach(item => {
            ...
            let subtitle = isPdfManuscript ? `${mainBook.total_pages || 0} لوحة` : (isSeries ? `${booksInGroup.length} أجزاء` : `${mainBook.total_pages || 0} صفحة`);
    ...
    volumesList.forEach(vol => {
        ...
        item.innerHTML = `
            <div style="display: flex; align-items: center; gap: 10px; overflow: hidden;">
                <i class="fas fa-book text-gold"></i><span class="toc-item-title" style="font-weight: bold; font-size: 14px;">${volLabel}</span>
            </div>
...
async function loadAndOpenBook(bookId, bookTitle, bookToc, totalPages, targetPageNumber = null, highlightQuery = "") {
    ...
    try {
        ...
        contentDiv.innerHTML = `<div style="text-align:center; color:#ff5252; padding:30px;">⚠️ تعذر فتح الكتاب (${bookId}.json). تأكد من رفع الأجزاء.</div>`;
    }
}
...
function renderCurrentPage() {
    ...
    if (currentActiveSearchHighlight) contentDiv.innerHTML = highlightArabicText(rawHtml, currentActiveSearchHighlight);
    else contentDiv.innerHTML = rawHtml;
    ...
    if (rangeSlider) { rangeSlider.min = 1; rangeSlider.max = currentBookPages.length; rangeSlider.value = currentPageIndex; }
    if (currentLbl) currentLbl.innerText = displayPage;
    if (totalLbl) totalLbl.innerText = currentBookTotalPages;

    if (currentBookId) { try { localStorage.setItem(`last_page_${currentBookId}`, currentPageIndex); } catch (e) {} }
...
function slidePageChanged(val) {
    ...
    if (!isNaN(idx) && idx >= 1 && idx <= currentBookPages.length) { currentPageIndex = idx; renderCurrentPage(); }
}
function closeReader() { 
    currentActiveSearchHighlight = "";
    if (history.state && history.state.view === 'readerView') history.back();
    else showView('homeView', false);
}
...
function applyHighlight(className) {
    ...
    try { savedSelectionRange.surroundContents(span); } 
    catch (e) { document.execCommand('backColor', false, className === 'hl-yellow' ? '#ffeb3b' : (className === 'hl-green' ? '#4caf50' : '#e91e63')); }
...
function shareSelectedQuote() {
    ...
    else { navigator.clipboard.writeText(quoteFormatted); showToast("تم نسخ الاقتباس مع المصدر", "fa-clipboard-check"); }
    document.getElementById('selectionToolbar').style.display = 'none';
    window.getSelection().removeAllRanges();
}
...
function openAddTagModal() {
    ...
    if (!text) { showToast("يرجى تحديد نص لتصنيفه أولاً", "fa-triangle-exclamation"); return; }
...
function createNewCustomTag() {
    ...
    if (!name) { showToast("يرجى كتابة اسم للوسم أولاً", "fa-triangle-exclamation"); return; }
    const tags = getStoredTags();
...
function assignTagToSelection(tagName, tagColor) {
    ...
    taggedItems.unshift({
        id: 'tag_' + Date.now(), tagName: tagName, tagColor: tagColor, text: savedSelectionText,
        bookId: currentBookId, bookTitle: currentBookTitle, pageNum: pageNum, pageIndex: currentPageIndex, date: new Date().toLocaleDateString('ar-IQ')
    });
...
function renderTagsView(filterTag = 'all') {
    ...
    if (filteredItems.length === 0) {
        listContainer.innerHTML = `<div class="search-empty-state"><div class="empty-icon-box"><i class="fas fa-tags text-gold"></i></div><h4>لا توجد نصوص موسومة في هذا التصنيف</h4></div>`;
        return;
    }
    ...
    filteredItems.forEach((item) => {
        ...
        card.innerHTML = `
            <div class="tagged-card-header"><span class="tag-pill-badge" style="background-color: ${item.tagColor || '#D4AF37'};"><i class="fas fa-tag"></i> ${item.tagName}</span><span style="font-size: 10px; color: var(--text-muted);">${item.date || ''}</span></div>
            <p class="tagged-quote-text">«${item.text}»</p>
            <div class="tagged-card-footer">
                <div><i class="fas fa-book-bookmark text-gold"></i> ${item.bookTitle} (صـ ${item.pageNum})</div>
...
function shareTaggedSnippet(itemId) {
    ...
    else { navigator.clipboard.writeText(shareContent); showToast("تم نسخ النص مع المصدر", "fa-clipboard-check"); }
}
...
function deleteTaggedSnippet(itemId) {
    showConfirm("هل أنت متأكد من الحذف؟", () => {
        ...
        renderTagsView(currentTagFilter);
        showToast("تم الحذف بنجاح", "fa-trash-can");
    });
}
...
function toggleBookmark() {
    ...
    if (existingIndex !== -1) {
        ...
        const tempDiv = document.createElement('div'); tempDiv.innerHTML = curPageData ? curPageData.content : '';
        const preview = (tempDiv.textContent || '').trim().substring(0, 50) + '...';
        bookmarks.push({ pageIndex: currentPageIndex, pageNum: curPageNum, preview: preview, date: new Date().toLocaleDateString('ar-IQ') });
...
function updateBookmarkIconState() {
    ...
    let bookmarks = getStoredBookmarks();
    btn.innerHTML = bookmarks.some(b => b.pageIndex === currentPageIndex) ? '<i class="fas fa-bookmark" style="color:#D4AF37;"></i>' : '<i class="far fa-bookmark"></i>';
}
    ...
    bookmarks.forEach((b, bIdx) => {
        ...
        div.innerHTML = `
            <div style="flex:1; overflow:hidden;">
                <div style="display:flex; justify-content:space-between; margin-bottom:2px;"><span class="toc-item-page">صفحة ${b.pageNum}</span><span style="font-size:10px; color:#888;">${b.date || ''}</span></div>
...
function switchModalTab(tab) {
    ...
    if (tab === 'toc') {
        tocTab.classList.add('active'); bmarksTab.classList.remove('active');
        tocList.style.display = 'block'; bmarksList.style.display = 'none';
    } else {
        bmarksTab.classList.add('active'); tocTab.classList.remove('active');
        tocList.style.display = 'none'; bmarksList.style.display = 'block';
    }
}
...
function renderTocList() {
    ...
    if (currentBookToc.length === 0) {
        tocContainer.innerHTML = '<div style="color:var(--text-muted); text-align:center; padding: 20px;">لا يوجد فهرس تفصيلي.</div>';
        return;
    }
    currentBookToc.forEach(item => {
        ...
        div.onclick = () => {
            ...
            currentPageIndex = targetIdx !== -1 ? (targetIdx + 1) : 1;
            currentActiveSearchHighlight = "";
            renderCurrentPage(); closeTocModal();
        };
...
function openInBookSearch() {
    ...
    if (modal) { modal.style.display = 'flex'; setTimeout(() => { document.getElementById('inBookSearchInput')?.focus(); }, 150); }
}
...
function executeInBookSearch(val) {
    ...
    if (!container) return;
    if (!query) { container.innerHTML = '<p style="text-align:center; color:var(--text-muted); padding:20px;">اكتب كلمة للبحث...</p>'; return; }
    ...
    currentBookPages.forEach((page, idx) => {
        ...
        if (searchRegex.test(rawText)) {
            ...
            attachTactilePhysics(item);
            item.onclick = () => { currentActiveSearchHighlight = query; currentPageIndex = idx + 1; renderCurrentPage(); closeInBookSearch(); };
            container.appendChild(item);
        }
    });
    if (found === 0) container.innerHTML = `<p style="text-align:center; color:var(--text-muted); padding:20px;">لا توجد نتائج.</p>`;
}
...
function generateSearchSnippet(fullText, rawQuery) {
    ...
    if (match) {
        ...
        let end = Math.min(fullText.length, match.index + match[0].length + 65);
        snippet = fullText.substring(start, end);
        if (start > 0) snippet = '...' + snippet;
        if (end < fullText.length) snippet = snippet + '...';
    } else { snippet = fullText.substring(0, 100) + '...'; }
    return highlightArabicText(snippet, rawQuery);
}

// ==================== البحث الشامل ====================
function openSearch() { showView('searchView'); setTimeout(() => { document.getElementById('searchInput')?.focus(); }, 150); }
...
async function executeGlobalSearch() {
    ...
    if (!query) {
        ...
        container.innerHTML = `<div class="search-empty-state"><h4>ابحث في أسماء المتون، الأبواب، أو نصوص الصفحات</h4></div>`;
        return;
    }
    ...
    if (currentSearchScope !== 'all' && currentSearchScope.startsWith('group:')) {
    ...
    if (currentSearchTarget === 'toc') {
        ...
        if (statusInfo && countBadge && filterBadge) { statusInfo.style.display = 'flex'; countBadge.innerText = `${foundCount} نتائج`; filterBadge.innerText = `${filterLabel} (أبواب)`; }
        if (foundCount === 0) container.innerHTML = `<div class="search-empty-state"><h4>لم نجد أبواباً مطابقة</h4></div>`;
    } 
    else if (currentSearchTarget === 'fulltext') {
        const progressIndicator = document.createElement('div');
        progressIndicator.className = "glass-box";
        progressIndicator.style.padding = "10px 14px"; progressIndicator.style.marginBottom = "10px"; progressIndicator.style.textAlign = "center"; progressIndicator.style.fontSize = "12px"; progressIndicator.style.color = "var(--gold-bright)";
        progressIndicator.innerHTML = `<i class="fas fa-spinner fa-spin"></i> جاري البحث... (<span id="deepSearchProgress">0%</span>)`;
        ...
        if (statusInfo && countBadge && filterBadge) { statusInfo.style.display = 'flex'; countBadge.innerText = `${foundCount} نتائج`; filterBadge.innerText = `${filterLabel} (نصوص)`; }
        if (foundCount === 0) container.innerHTML = `<div class="search-empty-state"><h4>لم نجد نصوصاً مطابقة</h4></div>`;
    }
}
...
function initDailyHadithSystem() {
    ...
    hadithIntervalTimer = setInterval(() => { if (dailyHadithCollection.length > 0) loadRandomDailyHadith(); }, 10000);
}
...
function shareDailyHadith() {
    ...
    if (navigator.share) navigator.share({ title: "إشراقة علوم العترة", text: shareContent }).catch(() => {});
    else { navigator.clipboard.writeText(shareContent); showToast("تم النسخ بنجاح", "fa-clipboard-check"); }
}

// ==================== نافذة الحواشي المنبثقة ====================
let footnoteTimeout;
